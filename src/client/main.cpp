...
int main(int argc, char **argv)
{
    ...
    // 初始化读写线程通信用的信号量
    sem_init(&_senaphore,0,0);
    // 连接服务器成功，启动专门接收消息的子线程
    ...
    for (;;)
    {
        ...
        case 2: // register业务
        {
            ...
            // 发送后直等待就可以，其实这里可以考虑不等待
            sem_wait(&_senaphore);
        }
        break;
        case 3: // quit业务
            close(clientfd);
            sem_destroy(&_senaphore);
            exit(0);
        default:
...
void readTaskHandler(int clientfd)
{
    for(;;)
    {
        ...
        if (-1 == len || 0 == len)
        {
            cerr << "\rServer disconnected! Press Enter to quit..." << endl;
            close(clientfd);
            return;
            // exit(-1);
        }
...
void mainMenu(int clientfd)
{
    ...
    while(isMainMenuRunning)
    {
        ...
        it->second(clientfd, commandbuf.substr(idx + 1, commandbuf.size() - idx)); // 调用命令处理
    }
}
